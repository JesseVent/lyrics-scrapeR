...
getArtists <- function() {
  ...
  artistLoop <- function(attributes) {
    library(rvest)
    library(purrr)
    library(tibble)
...
getLyrics <- function(url) {
  ...
  songs <- c(html_nodes(webpage, "#listAlbum") %>% html_nodes("a") %>% html_attr("href") %>%
    na.omit())
  ...
  abstracts <- function(attributes) {
    ...
    lyrics <- ifelse(page$result == "0", " ", c(page$result %>% html_nodes("br+ div") %>%
      html_text(trim = TRUE) %>% .[. != ""]))
...
# Start scraping your favourite Artists lyrics
cat("Retrieving all lyrics for artist")
slice <- subset(ARTISTS, search == artist)
artist.page <- slice$link
url <- paste0("https://web.archive.org/web/20170330154255/https://www.azlyrics.com/",
  artist.page)
lyrics <- getLyrics(url)

# Write lyrics to file
cleaned <- gsub("\\[[^\\]]*\\]", " ", lyrics$lyrics, perl = TRUE)
write.csv(cleaned, file = "/Users/macbookpro/Development/drake/FIFTY-Lyrics.txt",
